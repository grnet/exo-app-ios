...
final class DownloadedPackagesSQLLiteStoreTests: XCTestCase {
	...
	func testEmptyEmptyDb() throws {
		store.open()
		XCTAssertNil(store.package(for: "2020-06-13", country: "GR"))
	}
	...
	func testSettingDays() throws {
		...
		try store.set(country: "GR", day: "2020-06-12", etag: nil, package: package)
		let packageOut = store.package(for: "2020-06-12", country: "GR")
	...
	func testSettingHoursForDay() throws {
		store.open()
		XCTAssertTrue(store.hourlyPackages(for: "2020-06-12", country: "GR").isEmpty)
		...
		let hourlyPackagesDE = store.hourlyPackages(for: "2020-06-12", country: "GR")
